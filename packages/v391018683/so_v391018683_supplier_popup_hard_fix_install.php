...
if($css===false||$js===false||$newCss===false||$newJs===false){so18683_fail('Canlı veya payload dosyaları okunamadı.');}
if(strpos($css,SO_V18683_CSS_START)!==false&&strpos($js,SO_V18683_JS_START)!==false){
...
if(!copy($cssFile,$backupCss)||!copy($jsFile,$backupJs)){so18683_fail('Canlı CSS/JS yedeklenemedi.');}
...
try{
    ...
    $manifest['sha256_after']=array('css'=>hash_file('sha256',$cssFile),'js'=>hash_file('sha256',$jsFile));
...
so18683_page('Kurulum tamamlandı','<p>Tedarikçi Seç ve Yeni Tedarikçi Ekle popupları için gerçek ürün formu CSS/JS dosyaları güncellendi.</p><ul><li>Fixed header üstü yüksek katman</li><li>320–430 px mobil genişlik kilidi</li><li>Stacking-context atası sıfırlama</li><li>Uzun isim ve input taşma koruması</li><li>Arka sayfa kaydırma kilidi</li></ul><p><a href="/products/new">Ürün ekleme ekranını test et</a></p><p>Rollback: <code>/so_v391018683_supplier_popup_hard_fix_rollback.php?key=sesliofis391018683</code></p>',true);
